...
from adk_labs.adk_level9_rag.agent import root_agent
...
def render():
    ...
    st.markdown("### 📚 Grounding Capabilities:")
    st.markdown("1. **Local Doc Search**: Try asking 'What is the refund policy?'")
    st.markdown("2. **Live Web Search**: Try asking 'Who won the last major tennis tournament?'")
    st.markdown("3. **Combined**: Try asking 'Compare Acme Corp products with OpenAI models.'")
    
    thread_id = st.text_input("Active Session (Thread ID):", value="adk_rag_session_1")
    st.markdown("---")
    
    async def get_session_history():
        session = await st.session_state.adk_rag_session_service.get_session(
    ...
    if user_prompt := st.chat_input("Ask a grounded question..."):
        with st.chat_message("user"):
            st.markdown(user_prompt)
            
        with st.chat_message("assistant"):
            with st.spinner("Agent is retrieving knowledge..."):
                try:
                    runner = Runner(agent=root_agent, app_name="app", session_service=st.session_state.adk_rag_session_service)
                    user_msg = types.Content(role="user", parts=[types.Part(text=user_prompt)])
                    
                    async def run_runner():
                        await st.session_state.adk_rag_session_service.get_session(app_name="app", user_id="user", session_id=thread_id) or \
                        await st.session_state.adk_rag_session_service.create_session(app_name="app", user_id="user", session_id=thread_id)
